 ...
 * desde el sidebar. Hoy solo `transversal` tiene canvas real (spec 05 F1);
 * `entrevistaIa` muestra un placeholder hasta que se implemente (spec 06).
 */
export function CanvasHito({ hito, cursoId, asignacionId }: CanvasHitoProps) {
  if (hito === "transversal") {
    return <CanvasTransversal cursoId={cursoId} asignacionId={asignacionId} />
  }
