...
export function HistoricoHeader({ frescura, scopeHash, nombreCliente }: HistoricoHeaderProps) {
  return (
    <header className="flex flex-col gap-4">
      ...
      <div className="flex flex-col gap-2">
        ...
        <h1 className="text-h1 text-text-primary">
          Histórico por cliente<span className="text-aurora-violet">.</span>
        </h1>
        <p className="max-w-[720px] text-body text-text-secondary">
          Trayectoria de los cursos preparados para un cliente: cuántos colaboradores se
          presentaron, cuántos fueron aceptados y la tasa de aceptación a lo largo del tiempo.
        </p>
      </div>
      ...
      <div className="flex flex-wrap items-center gap-x-4 gap-y-2 text-caption text-text-tertiary">
        {nombreCliente && (
          <span className="inline-flex items-center gap-1.5">
            <span className="h-1.5 w-1.5 rounded-pill bg-aurora-violet" aria-hidden={true} />
            {nombreCliente}
          </span>
        )}
        {frescura && <span>Datos al {formatearFrescura(frescura)}</span>}
        {scopeHash && (
