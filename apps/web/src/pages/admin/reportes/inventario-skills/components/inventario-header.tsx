...
export function InventarioHeader({ frescura, scopeHash, umbralCumple }: InventarioHeaderProps) {
  return (
    <header className="flex flex-col gap-4">
      ...
      <div className="flex flex-col gap-2">
        ...
        <h1 className="text-h1 text-text-primary">
          Inventario de skills<span className="text-aurora-violet">.</span>
        </h1>
        <p className="max-w-[720px] text-body text-text-secondary">
          Mapa cualitativo del talento de la plantilla. Cada skill se segmenta en cuatro niveles —
          excelencia, sólido, en desarrollo y no cumple — para localizar fortalezas y brechas de un
          vistazo.
        </p>
      </div>
      ...
      <div className="flex flex-wrap items-center gap-x-4 gap-y-2 text-caption text-text-tertiary">
        ...
        {umbralCumple !== undefined && (
          <span>
            umbral de cumplimiento{" "}
            <span className="tabular font-mono text-text-secondary">{umbralCumple}</span>
          </span>
        )}
